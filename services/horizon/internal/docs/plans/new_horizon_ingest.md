...
2. A full ledger reader, which provides random and up-to-date access to the transaction sets & metadata in ledger closes. It is used to construct a history of events on the Stellar network, as well as keep ingestion up to date with new ledger closes.
